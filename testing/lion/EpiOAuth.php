...
class EpiOAuth
{
 ...
 public function __construct($consumerKey, $consumerSecret, $signatureMethod='HMAC-SHA1')
  ...
  /*
   * Returns an osm.org URL where the user browser must be redirected for accepting authorization
   * Includes a request-token. False if error.
   */
  ...
  /*
   * Calls osm.org with HTTP GET for a token that must be added to query
   * when redirecting the user for authorization
   */
  ...
  public function httpRequest($method = null, $url = null, $params = null)
  {
	...
	// FIXME: PUT can be removed probably, NOTES feature may need it
    if ($method=='PUT') {
		//harry's fudge. Take 'data' param out of the params list
		$put_data = $params['data'];
		unset($params['data']);
    ...
    switch($method)
    {
      ...
      case 'POST':
        ...
        break;
      case 'PUT':
        return $this->httpPut($url, $params, $put_data);
        break;
    }
  }
  ...
  {
    $params = func_get_args(); //FIXME: unused?
    $this->token = $token;
  ...
  protected function addOAuthHeaders(&$ch, $url, $oauthHeaders)
  {
    ...
    {
      $oauth .= "{$name}=\"{$value}\","; //lose the final ','
    }
    $_h[] = substr($oauth, 0, -1);
  ...
  //Harry added this one.
  protected function httpPut($url, $params = null, $put_data)
  {		  
    $ch = curl_init($url);
    $this->addOAuthHeaders($ch, $url, $params['oauth']);
    ...
    //curl_setopt($ch, CURLOPT_POSTFIELDS, $put_data ); // http_build_query($params['request']) . "&xml=" . urlencode($put_data) );
    //curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $resp  = $this->curl->addCurl($ch);
    return $resp;
  }
  ...
  protected function normalizeUrl($url = null)
  {
    ...
    $port=0;
    if (isset($urlParts['port']))  $port = intval($urlParts['port']);
  ...
  /**
   * Extends the $params array you want send to the OAtuh server
   * with oauth's required many parameters
   * adds necessary esacping, encoding, signing.
   *
   * @returns Array with two keys: 'request': array from $params, 'oauth': array of oauth values
   */
...
class EpiOAuthResponse
{
  ...
  {
    //print "RESPONSE CODE '" . $resp->code . "'<br>\n";
    $this->__resp = $resp;
  }
  ...
  public function __get($name)
  {
    if($this->__resp->code < 200 || $this->__resp->code > 299) {
      print "get($name) fail due to earlier response code '" . $this->__resp->code . "'<br>\n";
      return false; 
    }
